...
class Display(
    ...
    private fun drawGridElements(viewState: ViewState, canvas: Canvas) {
        for (y in 0 until viewState.gameState.game.grid.height)
            for (x in 0 until viewState.gameState.game.grid.width) {
                val screenX = offset.x + x * newTile
                val screenY = offset.y + y * newTile
                val offsetX =
                    (viewState.gameState.game.grid.space[y][x].background / NUMBER_COLUMNS_IMAGES_FON) * tile
                val offsetY =
                    (viewState.gameState.game.grid.space[y][x].background % NUMBER_ROWS_IMAGES_FON) * tile
        ...
        for (y in 0 until viewState.gameState.game.grid.height)
            for (x in 0 until viewState.gameState.game.grid.width)
                if (viewState.gameState.game.grid.space[y][x].block != 0) {
                    val screenX = offset.x + x * newTile
                    val screenY = offset.y + y * newTile
                    val offset: Vector = getOffset(viewState.gameState.game.grid.space[y][x])

                    canvas.drawBitmap(
                        bmpKv[viewState.gameState.game.grid.space[y][x].block - 1],
                        Rect(
                            offset.x * tile,
                            offset.y * tile,
                            offset.x * tile + tile,
    ...
    private fun drawCurrentFigure(viewState: ViewState, canvas: Canvas) {
        for (cell in viewState.gameState.game.currentFigure.figure.cells) {
            val screenX =
                offset.x + ((cell.vector.x + viewState.gameState.game.currentFigure.position.x) * newTile).toFloat()
            val screenY =
                offset.y + ((cell.vector.y + viewState.gameState.game.currentFigure.position.y) * newTile).toFloat()
            var oldY = 0
            var cY = screenY
            var nTile = newTile
    ...
    private fun drawCharacter(viewState: ViewState, canvas: Canvas) {
        val offset = viewState.gameState.game.character.getSprite() * tile
        val screenX =
            this.offset.x + (viewState.gameState.game.character.location.position.x * newTile).toFloat()
        val screenY =
            this.offset.y + (viewState.gameState.game.character.location.position.y * newTile).toFloat()
        canvas.drawBitmap(
            bmpCharacter,
            Rect(
                offset.x,
    ...
    private fun drawNextFigure(viewState: ViewState, canvasInfo: Canvas, color: Int) {
        ...
        val x = viewState.gameState.game.nextFigure.getWidth()
        val ostX = 4 - x
        val porX = ostX / 2
        val pixX = porX * oneTile

        val y = viewState.gameState.game.nextFigure.getHeight()
        val ostY = 4 - y
        val porY = ostY / 2
        ...
        for (cell in viewState.gameState.game.nextFigure.cells) {
